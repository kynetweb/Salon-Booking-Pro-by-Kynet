...
class Sbprok_Posttypes {
	...
	public function save_booking_to_google($post_id) {
		...
		if ( 'sbprok_bookings' == get_post_type() ) 
		{
            ...
            if(!empty($event_id)){
			    $this->google_calendar->update_event($post_id,$emp_calendar_id,$event_id,$start_date,$end_time,$service,$customer_name);
			}else{
			    $this->google_calendar->create_event($post_id,$emp_calendar_id,$start_date,$end_time,$service,$customer_name);
			}
			}
		}
