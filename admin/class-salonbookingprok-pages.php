...
class Salonbookingprok_Pages {
	...
	function user_custom_fields( $user ) {
		...
		$active_status = get_the_author_meta( 'active_status', $user->ID);
	 	$profile_image = esc_attr( get_the_author_meta( 'profile_image', $user->ID ) );
		...
		<table class="form-table">
			...
			<tr>
				...
				<td>
                	<img src="<?php echo $profile_image; ?>" style="height:50px;">
                	<input type="text" name="profile_image" class="profile_image" class="regular-text"  value="<?php echo $profile_image; ?>" />
